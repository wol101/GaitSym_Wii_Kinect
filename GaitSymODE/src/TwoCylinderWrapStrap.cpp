...
void TwoCylinderWrapStrap::SetCylinderBody(Body *body)
{
    m_CylinderBody = body;
    m_PointForceList[2]->body = m_CylinderBody;
    m_PointForceList[3]->body = m_CylinderBody;
}
...
void TwoCylinderWrapStrap::Calculate(double deltaT)
{
    ...
    pgd::Vector worldCylinder1Position = QVRotate(qCylinderBody, m_Cylinder1Position) + vCylinderBody;
    pgd::Vector worldCylinder2Position = QVRotate(qCylinderBody, m_Cylinder2Position) + vCylinderBody;

    // now rotate so the cylider axes are lined up on the z axis
    pgd::Vector cylinderOriginPosition = QVRotate(qCylinderQuaternionInv, QVRotate(qCylinderBodyInv, worldOriginPosition));
    pgd::Vector cylinderInsertionPosition = QVRotate(qCylinderQuaternionInv, QVRotate(qCylinderBodyInv, worldInsertionPosition));
    pgd::Vector cylinderCylinder1Position = QVRotate(qCylinderQuaternionInv, QVRotate(qCylinderBodyInv, worldCylinder1Position));
    pgd::Vector cylinderCylinder2Position = QVRotate(qCylinderQuaternionInv, QVRotate(qCylinderBodyInv, worldCylinder2Position));
    ...
    theCylinder1Force = QVRotate(qCylinderBody, QVRotate(m_CylinderQuaternion, theCylinder1Force));
    theCylinder1ForcePosition = QVRotate(qCylinderBody, QVRotate(m_CylinderQuaternion, theCylinder1ForcePosition));
    theCylinder2Force = QVRotate(qCylinderBody, QVRotate(m_CylinderQuaternion, theCylinder2Force));
    theCylinder2ForcePosition = QVRotate(qCylinderBody, QVRotate(m_CylinderQuaternion, theCylinder2ForcePosition));
    ...
    {
        m_PathCoordinates[i] = QVRotate(qCylinderBody, QVRotate(m_CylinderQuaternion, m_PathCoordinates[i]));
    }
...
#ifdef USE_OPENGL
void TwoCylinderWrapStrap::Draw()
{
    ...
    if (gSimulation->GetTime() != m_LastDrawTime)
    {
        ...
        dVector3 position;
        dBodyGetRelPointPos(m_CylinderBody->GetBodyID(), m_Cylinder1Position.x, m_Cylinder1Position.y, m_Cylinder1Position.z, position);
        // and draw it
        polyline.clear();
        ...
        dBodyGetRelPointPos(m_CylinderBody->GetBodyID(), m_Cylinder2Position.x, m_Cylinder2Position.y, m_Cylinder2Position.z, position);
        // and draw it
        polyline.clear();
        polyline.push_back(pgd::Vector(position[0] - cylinderVecWorld.x, position[1] - cylinderVecWorld.y, position[2] - cylinderVecWorld.z));
        polyline.push_back(pgd::Vector(position[0] + cylinderVecWorld.x, position[1] + cylinderVecWorld.y, position[2] + cylinderVecWorld.z));
