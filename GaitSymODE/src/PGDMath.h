...
public:
    ...
    inline double AngleBetweenNormalised(Vector v1, Vector v2);
    ...
    inline  void  Vector::Normalize(void)
    {
        ...
        // this process is really only necessary with single precision
        ...
        z /= m;
#else
        // with double precision we can just divide by the magnitude
        pgd_floating_point_type m = sqrt(x*x + y*y + z*z);
        if (m < (10 * DBL_MIN)) // cannot normalise a zero vector so just return an arbitrary vector
        {
            x = 1;
            y = 0;
            z = 0;
        ...
        // and just divide everything by the magnitude to normalise
        x /= m;
        y /= m;
        z /= m;
#endif
    ...
    // this routine returns the angle between two vectors using the dot product
    inline double AngleBetween(Vector v1, Vector v2)
    {
        pgd_floating_point_type cos_theta = (v1 * v2) / (v1.Magnitude() * v2.Magnitude());
        return acos(cos_theta);
    }

    // this routine returns the angle between two vectors using the dot product
    // but this version requires that the vectors are normalised before it is called
    inline double AngleBetweenNormalised(Vector v1, Vector v2)
    {
        pgd_floating_point_type cos_theta = (v1 * v2);
        return acos(cos_theta);
    }
