...
void DataTargetVector::Dump()
{
    ...
    if (m_DumpStream)
    {
        int valueListIndex = Util::BinarySearchRange(m_TargetTimeList, m_TargetTimeListLength, gSimulation->GetTime());
        if ((body = dynamic_cast<Body *>(m_Target)) != 0)
        {
            r = body->GetPosition();
            err = (pgd::Vector(r[0], r[1], r[2]) - m_ValueList[valueListIndex]).Magnitude();
...
void DataTargetVector::Draw()
{
    ...
    else
    {
        ...
        if (gAxisFlag)
        {
            ...
            GLUtils::DrawAxes(mRadius * 10, mRadius * 10, mRadius * 10, m_ValueList[valueListIndex].x, m_ValueList[valueListIndex].y, m_ValueList[valueListIndex].z);
        }
    }
}
#endif
