...
int Warehouse::XMLLoad(xmlNodePtr node)
{
    char buffer[128];
    ...
    while (true)
    {
        ...
        n = Util::CountTokens(attr->children->content);
        activations = new double[n];
        Util::Double(attr->children->content, n, activations);
        ...
        Util::Double(attr->children->content, n, bodyData);
        ...
        Util::Double(attr->children->content, n, weights);
...
xmlNodePtr Warehouse::XMLSave()
{
    xmlNodePtr newNode = xmlNewNode(0, (const xmlChar *)"WAREHOUSE");
    xmlNewProp(newNode, (const xmlChar *)"ID", (const xmlChar *)GetName()->c_str());
    xmlNewProp(newNode, (const xmlChar *)"UsePCA", m_UsePCA ? (const xmlChar *)"true" : (const xmlChar *)"false");
    ...
    xmlNewProp(newNode, (const xmlChar *)"DriverIDs", (const xmlChar *)ss.str().c_str());
    ...
    xmlNewProp(newNode, (const xmlChar *)"BodyIDs", (const xmlChar *)ss.str().c_str());
    ...
    for (unsigned int i = 0; i < GetNumWarehouseUnits(); i++)
    {
        ...
        ss2.str(""); ss2.clear();
        ss2 << "Activations" << i;
        xmlNewProp(newNode, (const xmlChar *)ss2.str().c_str(), (const xmlChar *)ss.str().c_str());
...
void Warehouse::Dump()
{
    ...
    if (m_DumpStream)
    {
        *m_DumpStream << gSimulation->GetTime() << "\t" << m_CurrentUnit << "\t" << m_LastSearchResult;
