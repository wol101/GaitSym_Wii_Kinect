...
def main():
    """Custom written version to take a 5 fixed phase genome and convert it into a 
    double length genome that allows co-contraction"""
    ...
    for i in range(0, populationLength):
        ...
        # print genomeLength
        c = c + 1
        if (genomeLength - 1) % 6 != 0:
            PrintExit("(genomeLength - 1) must be a multiple of 6")
        ...
        outpop.append(-1)
        outpop.append((genomeLength - 1) * 2 + 1) # genome length
        outpop.append(genome[0]) # phase duration
        outpop.append(genomeLow[0])
        outpop.append(genomeHigh[0])
        outpop.append(genomeSD[0])
        
        # split the flexion and extension values
        ...
        for j in range(1, genomeLength):
            if genome[j] >= 0:
                ...
                outpop.append(0)
            outpop.append(0)
            outpop.append(genomeHigh[j])
            outpop.append(genomeSD[j])
                
        for j in range(1, genomeLength):
            if genome[j] < 0:
                outpop.append(genome[j] * -1)
            else:
                outpop.append(0)
            outpop.append(0)
            outpop.append(genomeHigh[j])
        ...
        # sadd the extra stuff at the end
