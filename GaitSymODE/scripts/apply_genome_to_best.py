...
def main():
    usageString = "Usage:\n%s [-r root_config_file] [-e apply_genome_executable] [-f working_folder]" % sys.argv[0]
    ...
    if (len(name) > 0):
        ...
        command = '"%s" --inputXML "%s" --inputGenome "%s" --outputXML "%s.xml"' % (apply_genome_executable, root_config_file, bestGenome, bestGenome)
        print command
        os.system(command)
