...
def main():
    ...
    # compulsory values
    try:
        repeatCount = int(GetValueFromTokenList(tokenList, "repeatCount"))
    ...
    dirName = range(0, repeatCount)
    for i in range(0, repeatCount):
    
        # create the directory name
        theTime = time.localtime(time.time())
        dirName[i] = "%s%04d/Run_%04d-%02d-%02d_%02d.%02d.%02d" % (dirPrefix, i, theTime[0], theTime[1], theTime[2], theTime[3], theTime[4], theTime[5])
        os.makedirs(dirName[i])
        
        # run the ga command
        command = "%s -p %s -o %s" % (gaCommand, parameterFile, dirName[i])
        print command
        ...
        for name in fileList:
            if StartsWith(name, "BestGenome_") and EndsWith(name, ".xml"): 
                bestConfigFile = dirName[i] + "/" + name
                print "Best config file is %s" % (bestConfigFile)
                break
                
        # run the objective

        command = "%s -t 0.00000000001 -c %s -S %d -K %s" % (objectiveCommand, bestConfigFile, kinematicsSkip, dirName[i] + "/" + kinematicsFile)
        print command
        if os.system(command): 
            PrintExit("Error running %s" % command)  
