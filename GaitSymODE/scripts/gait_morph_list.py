...
def main():
    usageString = "Usage:\n%s [--controlFile file] [--dirPrefix dir] -[--restart sourceDir repeatIndex controlIndex]" % sys.argv[0]
    ...
    while i < len(sys.argv):
        ...
        if sys.argv[i] == "--restart":
            ...
            i = i + 1
            if i >= len(sys.argv): PrintExit(usageString)
            repeatIndex = int(sys.argv[i]) 
            i = i + 1
            if i >= len(sys.argv): PrintExit(usageString)
            controlIndex = int(sys.argv[i]) 
            i = i + 1
            restartFlag = 1
            continue 
    ...
    # compulsory values
    try:
        kRangeControlList = GetNumberListFromTokenList(tokenList, "rangeControlList")
        ...
        randomiseModel = int(GetValueFromTokenList(tokenList, "randomiseModel"))
    ...
    if restartFlag == 0:
        ...
        for name in fileList:
            if StartsWith(name, "BestGenome_") and EndsWith(name, ".txt"): 
                print "Copying %s to %s" % (sourceDir + "/" + name, workingGenome)
                CopyFile(sourceDir + "/" + name, workingGenome)
                break

        for name in fileList:
            if StartsWith(name, "BestGenome_") and EndsWith(name, ".xml"): 
                print "Copying %s to %s" % (sourceDir + "/" + name, workingConfigFile)
                CopyFile(sourceDir + "/" + name, workingConfigFile)
                break

        for name in fileList:
            if StartsWith(name, "Population_") and EndsWith(name, ".txt"): 
                print "Copying %s to %s" % (sourceDir + "/" + name, workingPopulation)
                CopyFile(sourceDir + "/" + name, workingPopulation)
                break
        ...
        print "Copying %s to %s" % (workingModelState, sourceDir + "/" + workingModelState)    
        CopyFile(workingModelState, sourceDir + "/" + workingModelState);
        print "Copying %s to %s" % (kinematicsFile, sourceDir + "/" + kinematicsFile)    
        CopyFile(kinematicsFile, sourceDir + "/" + kinematicsFile);
                
        repeatCount = repeatIndex
        kRangeControlIndex = controlIndex
        firstTimeFlag = 0
        
    
    while kRangeControlIndex < len(kRangeControlList):
        kRangeControl = kRangeControlList[kRangeControlIndex] 
        ...
        if usePopulationFlag: 
            ...
            theOutput.write("modelGenome %s\n\n" % workingGenome)
        theOutput.write("extraDataFile %s\n\n" % workingConfigFile)
        if firstTimeFlag != 0:
            theOutput.write("randomiseModel %d\n\n" % randomiseModel) # only randomise the first time through
            firstTimeFlag = 0
        else:
            theOutput.write("randomiseModel %d\n\n" % 0)
        ...
        while fitnessMinimumRepeatCount < fitnessMinimumRepeats:
            ...
            if score >= fitnessMinimum:
                ...
                if os.path.exists("StopFile"):
                    PrintExit("'StopFile' found. Run terminated.")  
        ...
        if repeatCount >= repeatsAtEachValue:
            repeatCount = 0;
            kRangeControlIndex = kRangeControlIndex + 1
