...
def main():
    """Read in a fixed interval genome and change the cycle time limits
    and also make sure the actual cycle time fits the limits"""
    ...
    for i in range(0, populationLength):
        ...
        cycleDuration = genome[0]
        if (cycleDuration < lowerLimit):
            cycleDuration = lowerLimit
        if (cycleDuration > upperLimit):
            cycleDuration = upperLimit
        outpop.append(cycleDuration) 
    ...
    WriteTokenList(fileName + "_change_timing", outpop)
