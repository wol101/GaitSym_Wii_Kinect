...
def main():
    ...
    And lists of any sort can use operator Index
    [[token Index i]] to obtain just the index offset
    ...
    while (match != None):
        ...
        if (len(tokenList) == 3):
            try:
                if (tokenList[1] == "+"): result = float(tokenList[0]) + float(GetValueFromTokenList(controlTokenList, tokenList[2]))
                elif (tokenList[1] == "-"): result = float(tokenList[0]) - float(GetValueFromTokenList(controlTokenList, tokenList[2]))
                elif (tokenList[1] == "*"): result = float(tokenList[0]) * float(GetValueFromTokenList(controlTokenList, tokenList[2]))
                elif (tokenList[1] == "/"): result = float(tokenList[0]) / float(GetValueFromTokenList(controlTokenList, tokenList[2]))
                elif (tokenList[1] == "Index"):
                    coordsList = ConvertToListOfTokens(GetValueFromTokenList(controlTokenList, tokenList[0]))
                    index = int(tokenList[2])
                    if (len(coordsList) <= index):
                        PrintExit("Error parsing token %s" % (match.group()))
                    result = float(coordsList[index])
                else:
                    PrintExit("Unrecognised operator in token %s" % (match.group()))
